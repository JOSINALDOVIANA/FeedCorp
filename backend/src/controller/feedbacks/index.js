  ...
  async get(req, res) {
    ...
    try {
      ...
      if (id_user) {
        ...
        let feedbacks_serial = [];
        for (let iterator of feedbacks) {
          iterator = {
            ...iterator,
            destinatario: await conexao("users").where({"id":iterator.id_direction}).first().select("users.name"),
            unidade: await conexao("units").where({"id":iterator.id_unity}).first().select("units.initials")
          }
          feedbacks_serial.push(iterator);

        }
        feedbacks = feedbacks_serial;
