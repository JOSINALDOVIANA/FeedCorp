export function up(knex) {
    return  knex.schema.createTable('feedbacks', function (table) {
        ...
        table.foreign("id_unity").references("id").inTable("units");
        table.foreign("id_company").references("id").inTable("companies").onDelete("cascade");
        table.foreign("id_direction").references("id").inTable("users");
        table.foreign("id_type").references("id").inTable("typesfeedbacks");
